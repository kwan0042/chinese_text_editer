...
import { Download, FileText, Smartphone, PenTool, Share } from "lucide-react"; // 新增 Share icon
...
const App: React.FC = () => {
  ...
  const [canShare, setCanShare] = useState(false); // 新增：狀態來判斷是否支援分享
  ...
  useEffect(() => {
    ...
    // 檢查瀏覽器是否支援 Web Share API (通常在手機瀏覽器支援)
    if (navigator.share) {
      setCanShare(true);
    }
  ...
  const handleDownload = async () => {
    const fileName = `${docState.subject.text || "文件"}.pdf`;

    if (canShare) {
      // 手機/平板模式：嘗試呼叫原生分享
      try {
        // 1. 要求 generatePDF 回傳 Blob
        const blob = await generatePDF("pdf-content", fileName, {
          returnBlob: true,
        });
        ...
        if (blob instanceof Blob) {
          ...
          if (navigator.canShare && navigator.canShare({ files: [file] })) {
            ...
            return; // 若分享成功，則不執行後續的下載
          }
        }
      } catch (error) {
        console.warn("Share failed or was canceled:", error);
        // 如果分享失敗（例如用戶按取消），則繼續執行下方的標準下載作為後備方案
      }
    }
  ...
  return (
    <div className="min-h-screen flex flex-col max-w-[1600px] mx-auto">
      ...
      <header className="bg-white border-b border-slate-200 sticky top-0 z-50">
        <div className="max-w-7xl mx-auto px-4 h-16 flex items-center justify-between">
          ...
          >
            ...
            <span>{canShare ? "分享 PDF" : "輸出 PDF"}</span>
          </button>
        </div>
      </header>
      ...
      <main className="flex-grow p-4 md:p-6 lg:p-8 flex flex-col lg:flex-row gap-6 lg:gap-8 overflow-hidden">
        ...
        <div
          className={`lg:w-1/3 lg:min-w-[400px] flex-col gap-4 overflow-y-auto pb-20 ${
            activeTab === "preview" ? "hidden lg:flex" : "flex"
          }`}
        >
          <h2 className="text-xl font-bold text-slate-800 mb-2 hidden lg:block">
            內容編輯
          </h2>
        ...
        <div
          className={`lg:flex-1 bg-slate-100 rounded-2xl flex flex-col items-center justify-start pt-4 lg:pt-8 overflow-y-auto border-2 border-dashed border-slate-200 ${
